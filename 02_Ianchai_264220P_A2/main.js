...
const navihead = document.querySelector("#navihead");
...
navihead.addEventListener("click", function() { /* refresh browser */
...
/* preset colors for mobile */
function setPresetColor(hex){

    currentColor = hex;
    erasing = false;

    color.forEach(function(picker){
        picker.value = hex;
    });
...
presetButtons.forEach(function(btn){
	btn.addEventListener("click", function(){
		...
		setPresetColor(btn.dataset.color);

	});

});

function showPresetColors(){

    if(window.innerWidth > 800){ /* only shows presets on mobile */
        return;
    }

    presetButtons.forEach(function(btn,index){

        btn.style.background = levelColors[currentLevel][index];
        btn.dataset.color = levelColors[currentLevel][index];

    });

}

/* preset color for mobile users */
const levelColors = [
...
const fsBtn2 = document.querySelector("#fsBtn2");
fsBtn.addEventListener("click",enterFullscreen);
fsBtn2.addEventListener("click",exitFullscreen);
